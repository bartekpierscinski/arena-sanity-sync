...
This package provides the **core logic only** — no Nuxt integration, no Studio UI. Use it to build your own adapter (Next.js, Cloudflare Workers, CLI, etc.).

---

## Features

- 🔗 **Sync Are.na → Sanity**: Keep blocks mirrored as documents.
- 🔑 **Idempotent updates**: Only patches changed blocks (uses fingerprint & updated_at).
- 🖼️ **Image upload modes**: Control whether to import images to Sanity or just store URLs.
- ⚡ **Timeouts & retries**: Robust against network hiccups.
- 🧩 **Framework-agnostic**: Works in Node, serverless, or custom cron.
- 🛡️ **Field protection**: Only touches whitelisted `arena*` fields; respects `lockAll`/`lockImage`.
...
- 🛡️ Field protection: Only touches whitelisted `arena*` fields; respects `lockAll` and `lockImage` when present.
